...
USE `Machimba`$$
DROP TRIGGER IF EXISTS `Machimba`.`purchase_has_product_BEFORE_UPDATE` $$
USE `Machimba`$$
CREATE DEFINER = CURRENT_USER TRIGGER `Machimba`.`purchase_has_product_BEFORE_UPDATE` BEFORE UPDATE ON `purchase_has_product` FOR EACH ROW
BEGIN
    DECLARE product_price INT;
