...
USE `Machimba`$$
DROP TRIGGER IF EXISTS `Machimba`.`purchase_AFTER_UPDATE` $$
USE `Machimba`$$
CREATE DEFINER = CURRENT_USER TRIGGER `Machimba`.`purchase_AFTER_UPDATE` AFTER UPDATE ON `purchase` FOR EACH ROW
BEGIN
